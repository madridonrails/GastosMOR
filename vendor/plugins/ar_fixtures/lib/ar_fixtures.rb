...
class ActiveRecord::Base
  ...
  class << self
    ...
    def to_fixture(limit=nil, key_column=nil, use_prefix=false)
      opts = {}
      opts[:limit] = limit if limit

      prefix = use_prefix ? key_column + '_' : ''

      write_file(File.expand_path("test/fixtures/#{table_name}.yml", RAILS_ROOT), 
          self.find(:all, opts).inject({}) { |hsh, record| 
              hsh.merge(!key_column.nil? && record.respond_to?(key_column) ? "#{prefix}#{record.send(key_column).to_s.downcase.gsub(' ', '_')}" : "#{table_name.singularize}_#{'%05i' % record.id}" => record.attributes) 
            }.ya2yaml(:SortKeys => true))
      habtm_to_fixture
    end
