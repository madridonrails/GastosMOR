...
module BundledResource::DynarchCalendar
  ...
  module Helper
    ...
    def dynarch_date_select(object_name, method_name, options = {})
      ...
      buffer << "<script type=\"text/javascript\">convert_date_container_to_dynarch_calendar('#{object_name}', '#{method_name}', #{index}, '#{initial_date}', '#{initial_display}', '#{image_url}')</script>\n"
    end
